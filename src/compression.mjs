...
export async function compressImage(fileOrBlob, quality = 0.85, options = { skipThresholdBytes: 102400 }) {
  ...
  const isWebp = fileOrBlob.type === 'image/webp' || (fileOrBlob.name && fileOrBlob.name.toLowerCase().endsWith('.webp'));
  if (isWebp && originalSize <= options.skipThresholdBytes) {
    return { blob: fileOrBlob, originalSize, newSize: originalSize, skipped: true };
  }
